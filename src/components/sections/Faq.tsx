...
function FaqItem({
  ...
  return (
    <button
      ...
      onClick={onToggle}
      aria-expanded={open}
      className={`relative w-[862px] text-left overflow-hidden rounded-[5px] transition-all duration-400 ${
        open
          ? "h-56 bg-yellow-50 outline outline-2 outline-offset-[-2px] outline-yellow-400"
          : "h-20 bg-white hover:bg-yellow-50/50"
      }`}
    >
      ...
      <div className="absolute right-6 top-[23px] flex size-8 items-center justify-center transition-transform duration-300">
        <div className="relative size-5">
          {!open && (
            <div className="absolute left-1/2 top-0 h-5 w-[2.67px] -translate-x-1/2 bg-black transition-all duration-300" />
          )}
